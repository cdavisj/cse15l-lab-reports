...
Some notes to help you understand this command. You can break commands up in the command line with semicolons (;). So each time I put a semicolon, I can put a new command afterwards that I would like to be run immediately after the first. Furthermore, you can wrap all the commands you would like to run on the server, after you `ssh` in, in double-quotes. When first applying this command, you can simply copy-paste it into the command line before pressing enter. This would take a total of 5 keystrokes (ctrl+c, ctrl+v, enter). Although, once you have entered this at least once, you just have to press the up-arrow followed by enter to re-run the command. This would take it down to 2 keystrokes (up, enter) to send, compile, and run a java program from your local machine over on a remote server.
